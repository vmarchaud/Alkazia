...
public class EntitySheep extends EntityAnimal {
    ...
    protected void dropDeathLoot(boolean flag, int i) {
        ...
        // Alkazia - set sheep lotting
        int j = this.random.nextInt(3) + 1 + this.random.nextInt(1 + i);
        
        if (j > 0) {
            if (this.isBurning()) {
                loot.add(new org.bukkit.inventory.ItemStack(CraftMagicNumbers.getMaterial(Items.cooked_sheep), j));
            } else {
                loot.add(new org.bukkit.inventory.ItemStack(CraftMagicNumbers.getMaterial(Items.sheep), j));
            }
        }
