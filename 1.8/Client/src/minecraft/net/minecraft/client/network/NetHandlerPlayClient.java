...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void handleChat(S02PacketChat packetIn)
    {
        ...
        else
        {
        	...
        	// Alkazia - handle information in chat
        	
        	if(packetIn.func_148915_c().getUnformattedText().startsWith("[SERVER]")) {
        		String[] str = packetIn.func_148915_c().getUnformattedText().split("]");
        		
        		gameController.server = serverType.valueOf(str[1]);
        	}
        	else if(packetIn.func_148915_c().getUnformattedText().startsWith("[Alkazia]") && packetIn.func_148915_c().getUnformattedText().contains("bon jeu sur Alkazia !")) {
                this.gameController.ingameGUI.getChatGUI().printChatMessage(packetIn.func_148915_c());
        		this.gameController.thePlayer.sendChatMessage("/launcher " + MD5(String.valueOf(gameController.getSession().getUsername().hashCode())));
        	}
        	else if((packetIn.func_148915_c().getUnformattedText().contains("[+]") || packetIn.func_148915_c().getUnformattedText().contains("[-]")) && gameController.clearChat) {}
        	else if(packetIn.func_148915_c().getUnformattedText().contains("[Death]") && gameController.clearChat) {}
        	
        	else if(packetIn.func_148915_c().getUnformattedText().startsWith("[BOSSBAR]:")) {
        		String[] info = packetIn.func_148915_c().getFormattedText().split(":");
        		BossStatus.setBossStatus(Float.valueOf(info[1]), info[2]);
        	}
        	else if(packetIn.func_148915_c().getUnformattedText().startsWith("[BOSSBAR]REMOVE")) {
        		BossStatus.resetBossStatus();
        	}
        	else {
               this.gameController.ingameGUI.getChatGUI().printChatMessage(packetIn.func_148915_c());
        	}
