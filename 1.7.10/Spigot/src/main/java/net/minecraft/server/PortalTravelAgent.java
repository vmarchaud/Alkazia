...
public class PortalTravelAgent {
		...
		for (int l = -2; l <= 2; ++l) {
			for (int i1 = -2; i1 <= 2; ++i1) {
				for (int j1 = -1; j1 < 3; ++j1) {
					...
					a.setTypeUpdate(k1, l1, i2, flag ? Blocks.OBSIDIAN : Blocks.AIR);
				}
			}
		}
		...
		for (int l = -2; l <= 2; ++l) {
			for (int i1 = -2; i1 <= 2; ++i1) {
				for (int j1 = -1; j1 < 3; ++j1) {
					...
					if (a.getType(k1, l1, i2) != (flag ? Blocks.OBSIDIAN : Blocks.AIR))
						return null;
				}
			}
		}
	...
	public boolean b(Entity entity, double d0, double d1, double d2, float f) {
		...
		Location exit = new Location(a.getWorld(), found.x, found.y, found.z, f, entity.pitch);
	...
	public ChunkCoordinates findPortal(double x, double y, double z, int short1) {
		if (a.getWorld().getEnvironment() == org.bukkit.World.Environment.THE_END)
			return findEndPortal(a.worldProvider.h());
		// CraftBukkit end
		double d3 = -1.0D;
		int i = 0;
		int j = 0;
		...
		if (c.contains(j1)) {
			ChunkCoordinatesPortal chunkcoordinatesportal = (ChunkCoordinatesPortal) c.getEntry(j1);

			d3 = 0.0D;
			i = chunkcoordinatesportal.x;
			j = chunkcoordinatesportal.y;
			k = chunkcoordinatesportal.z;
			chunkcoordinatesportal.d = a.getTime();
			flag = false;
		} else {
			for (k1 = l - short1; k1 <= l + short1; ++k1) {
				double d5 = k1 + 0.5D - x; // CraftBukkit

				for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1) {
					double d6 = l1 + 0.5D - z; // CraftBukkit

					for (int i2 = a.S() - 1; i2 >= 0; --i2) {
						if (a.getType(k1, i2, l1) == Blocks.PORTAL) {
							while (a.getType(k1, i2 - 1, l1) == Blocks.PORTAL) {
								--i2;
							}

							d4 = i2 + 0.5D - y; // CraftBukkit
		...
		if (d3 >= 0.0D) {
			if (flag) {
				c.put(j1, new ChunkCoordinatesPortal(this, i, j, k, a.getTime()));
				d.add(Long.valueOf(j1));
			}
	...
	public void adjustExit(Entity entity, Location position, Vector velocity) {
		...
		if (a.getWorld().getEnvironment() == org.bukkit.World.Environment.THE_END) {
			...
			if (a.getType(i - 1, j, k) == Blocks.PORTAL) {
				j2 = 2;
			}
			...
			if (a.getType(i, j, k - 1) == Blocks.PORTAL) {
				j2 = 3;
			}
			...
			if (j2 > -1) {
				...
				boolean flag1 = !a.isEmpty(i + i3 + k3, j, k + j3 + l3) || !a.isEmpty(i + i3 + k3, j + 1, k + j3 + l3);
				boolean flag2 = !a.isEmpty(i + i3, j, k + j3) || !a.isEmpty(i + i3, j + 1, k + j3);
				...
				if (flag1 && flag2) {
					...
					d4 -= l3;
					flag1 = !a.isEmpty(k1 + i3 + k3, j, i4 + j3 + l3) || !a.isEmpty(k1 + i3 + k3, j + 1, i4 + j3 + l3);
					flag2 = !a.isEmpty(k1 + i3, j, i4 + j3) || !a.isEmpty(k1 + i3, j + 1, i4 + j3);
				}
	...
	public boolean createPortal(double x, double y, double z, int b0) {
		...
		for (i2 = i - b0; i2 <= i + b0; ++i2) {
			...
			for (j2 = k - b0; j2 <= k + b0; ++j2) {
				...
				label274: for (k2 = a.S() - 1; k2 >= 0; --k2) {
					if (a.isEmpty(i2, k2, j2)) {
						while (k2 > 0 && a.isEmpty(i2, k2 - 1, j2)) {
							--k2;
						}
							...
							for (j3 = 0; j3 < 3; ++j3) {
								for (i4 = 0; i4 < 4; ++i4) {
									for (l3 = -1; l3 < 4; ++l3) {
										...
										if (l3 < 0 && !a.getType(k4, j4, l4).getMaterial().isBuildable() || l3 >= 0 && !a.isEmpty(k4, j4, l4)) {
											continue label274;
										}
									}
								}
							...
							for (i3 = l1; i3 < l1 + 2; ++i3) {
								...
								for (j3 = 0; j3 < 4; ++j3) {
									for (i4 = -1; i4 < 4; ++i4) {
										...
										k4 = k2 + i4;
										j4 = j2 + (j3 - 1) * k3;
										if (i4 < 0 && !a.getType(l3, k4, j4).getMaterial().isBuildable() || i4 >= 0 && !a.isEmpty(l3, k4, j4)) {
											continue label222;
										}
									}
								}
			...
			for (k2 = -1; k2 <= 1; ++k2) {
				for (i3 = 1; i3 < 3; ++i3) {
					for (l2 = -1; l2 < 3; ++l2) {
						...
						i4 = j2 + (i3 - 1) * l5 - k2 * k5;
						flag = l2 < 0;
						a.setTypeUpdate(k3, j3, i4, flag ? Blocks.OBSIDIAN : Blocks.AIR);
					}
				}
			}
		}
		...
		for (k2 = 0; k2 < 4; ++k2) {
			for (i3 = 0; i3 < 4; ++i3) {
				for (l2 = -1; l2 < 4; ++l2) {
					...
					a.setTypeAndData(k3, j3, i4, flag ? Blocks.OBSIDIAN : Blocks.PORTAL, 0, 2);
				}
			}
			...
			for (i3 = 0; i3 < 4; ++i3) {
				for (l2 = -1; l2 < 4; ++l2) {
					...
					j3 = j5 + l2;
					i4 = j2 + (i3 - 1) * l5;
					a.applyPhysics(k3, j3, i4, a.getType(k3, j3, i4));
				}
			}
		}
	...
	public void a(long i) {
		if (i % 100L == 0L) {
			...
			while (iterator.hasNext()) {
				...
				ChunkCoordinatesPortal chunkcoordinatesportal = (ChunkCoordinatesPortal) c.getEntry(olong.longValue());

				if (chunkcoordinatesportal == null || chunkcoordinatesportal.d < j) {
					iterator.remove();
					c.remove(olong.longValue());
				}
			}
		}
	}
