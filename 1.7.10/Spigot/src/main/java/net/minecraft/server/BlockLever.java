...
public class BlockLever extends Block {
	...
	public boolean canPlace(World world, int i, int j, int k, int l) {
		return l == 0 && world.getType(i, j + 1, k).r() ? true : l == 1 && World.a(world, i, j - 1, k) ? true : l == 2 && world.getType(i, j, k + 1).r() ? true : l == 3 && world.getType(i, j, k - 1).r() ? true : l == 4 && world.getType(i + 1, j, k).r() ? true : l == 5 && world.getType(i - 1, j, k).r();
	}

	@Override
	public boolean canPlace(World world, int i, int j, int k) {
		return world.getType(i - 1, j, k).r() ? true : world.getType(i + 1, j, k).r() ? true : world.getType(i, j, k - 1).r() ? true : world.getType(i, j, k + 1).r() ? true : World.a(world, i, j - 1, k) ? true : world.getType(i, j + 1, k).r();
	}
	...
	public void postPlace(World world, int i, int j, int k, EntityLiving entityliving, ItemStack itemstack) {
		...
		if (i1 == b(1)) {
			if ((MathHelper.floor(entityliving.yaw * 4.0F / 360.0F + 0.5D) & 1) == 0) {
	...
	public void updateShape(IBlockAccess iblockaccess, int i, int j, int k) {
		...
		if (l == 1) {
			this.setBounds(0.0F, 0.2F, 0.5F - f, f * 2.0F, 0.8F, 0.5F + f);
		} else if (l == 2) {
			this.setBounds(1.0F - f * 2.0F, 0.2F, 0.5F - f, 1.0F, 0.8F, 0.5F + f);
		} else if (l == 3) {
			this.setBounds(0.5F - f, 0.2F, 0.0F, 0.5F + f, 0.8F, f * 2.0F);
		} else if (l == 4) {
			this.setBounds(0.5F - f, 0.2F, 1.0F - f * 2.0F, 0.5F + f, 0.8F, 1.0F);
		} else if (l != 5 && l != 6) {
			if (l == 0 || l == 7) {
				f = 0.25F;
				this.setBounds(0.5F - f, 0.4F, 0.5F - f, 0.5F + f, 1.0F, 0.5F + f);
			}
		} else {
			f = 0.25F;
			this.setBounds(0.5F - f, 0.0F, 0.5F - f, 0.5F + f, 0.6F, 0.5F + f);
		}
	}
