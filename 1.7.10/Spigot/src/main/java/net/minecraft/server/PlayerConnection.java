...
public class PlayerConnection implements PacketPlayInListener {
	...
	public CraftPlayer getPlayer() {
		return player == null ? null : (CraftPlayer) player.getBukkitEntity();
	}
	...
	public void a() {
		g = false;
		++e;
		minecraftServer.methodProfiler.a("keepAlive");
		if (e - k > 40L) {
			k = e;
			i = d();
			h = (int) i;
			sendPacket(new PacketPlayOutKeepAlive(h));
		}
		...
		if (player.x() > 0L && minecraftServer.getIdleTimeout() > 0 && MinecraftServer.ar() - player.x() > minecraftServer.getIdleTimeout() * 1000 * 60) {
	...
	public void disconnect(String s) {
		...
		String leaveMessage = EnumChatFormat.YELLOW + player.getName() + " left the game.";
		...
		networkManager.handle(new PacketPlayOutKickDisconnect(chatcomponenttext), new GenericFutureListener[] { new PlayerConnectionFuture(this, chatcomponenttext) });
	...
	public void a(PacketPlayInSteerVehicle packetplayinsteervehicle) {
		player.a(packetplayinsteervehicle.c(), packetplayinsteervehicle.d(), packetplayinsteervehicle.e(), packetplayinsteervehicle.f());
	}
	...
	public void a(PacketPlayInFlying packetplayinflying) {
		...
		if (!player.viewingCredits) {
			...
			if (!checkMovement) {
				d0 = packetplayinflying.d() - z;
				if (packetplayinflying.c() == y && d0 * d0 < 0.01D && packetplayinflying.e() == q) {
					checkMovement = true;
				}
			}
			...
			double delta = Math.pow(lastPosX - to.getX(), 2) + Math.pow(lastPosY - to.getY(), 2) + Math.pow(lastPosZ - to.getZ(), 2);
			float deltaAngle = Math.abs(lastYaw - to.getYaw()) + Math.abs(lastPitch - to.getPitch());

			if ((delta > 1f / 256 || deltaAngle > 10f) && checkMovement && !this.player.dead) {
				lastPosX = to.getX();
				lastPosY = to.getY();
				lastPosZ = to.getZ();
				lastYaw = to.getYaw();
			...
			if (checkMovement && !this.player.dead) {
				...
				if (this.player.vehicle != null) {
					...
					minecraftServer.getPlayerList().d(this.player);
					if (checkMovement) {
						y = this.player.locX;
						z = this.player.locY;
						q = this.player.locZ;
				...
				if (this.player.isSleeping()) {
					this.player.i();
					this.player.setLocation(y, z, q, this.player.yaw, this.player.pitch);
					worldserver.playerJoinedWorld(this.player);
					return;
				}
				...
				this.player.i();
				this.player.V = 0.0F;
				this.player.setLocation(y, z, q, f2, f3);
				if (!checkMovement)
					return;
				...
				if (d10 > org.spigotmc.SpigotConfig.movedTooQuicklyThreshold && checkMovement && (!minecraftServer.N() || !minecraftServer.M().equals(this.player.getName()))) { // CraftBukkit - Added this.checkMovement condition to solve this check being triggered by teleports
					c.warn(this.player.getName() + " moved too quickly! " + d4 + "," + d5 + "," + d6 + " (" + d7 + ", " + d8 + ", " + d9 + ")");
					this.a(y, z, q, this.player.yaw, this.player.pitch);
					return;
				}
				...
				if (!minecraftServer.getAllowFlight() && !this.player.abilities.canFly && !worldserver.c(axisalignedbb)) { // CraftBukkit - check abilities instead of creative mode
					if (d11 >= -0.03125D) {
						++f;
						if (f > 80) {
	...
	public void teleport(Location dest) {
		...
		player.playerConnection.sendPacket(new PacketPlayOutPosition(d0, d1 + 1.6200000047683716D, d2, f, f1, false));
	}
	...
	public void a(PacketPlayInBlockDig packetplayinblockdig) {
		...
		if (packetplayinblockdig.g() == 4) {
			...
			if (lastDropTick != MinecraftServer.currentTick) {
				...
				dropCount++;
				if (dropCount >= 20) {
					c.warn(player.getName() + " dropped their items too quickly!");
			...
			if (flag) {
				double d0 = player.locX - (i + 0.5D);
				double d1 = player.locY - (j + 0.5D) + 1.5D;
				double d2 = player.locZ - (k + 0.5D);
				double d3 = d0 * d0 + d1 * d1 + d2 * d2;

				if (d3 > 36.0D)
					return;

				if (j >= minecraftServer.getMaxBuildHeight())
					return;
			}

			if (packetplayinblockdig.g() == 0) {
				if (!minecraftServer.a(worldserver, i, j, k, player)) {
					player.playerInteractManager.dig(i, j, k, packetplayinblockdig.f());
				} else {
					// CraftBukkit start - fire PlayerInteractEvent
					CraftEventFactory.callPlayerInteractEvent(player, Action.LEFT_CLICK_BLOCK, i, j, k, packetplayinblockdig.f(), player.inventory.getItemInHand());
					player.playerConnection.sendPacket(new PacketPlayOutBlockChange(i, j, k, worldserver));
	...
	public void a(PacketPlayInBlockPlace packetplayinblockplace) {
		...
		if (org.clipspigot.ClipSpigotConfig.interactLimitEnabled && lastPlace != -1 && packetplayinblockplace.timestamp - lastPlace < 30 && packets++ >= 4) {
			throttled = true;
		...
		if (packetplayinblockplace.getFace() == 255) {
			if (packetplayinblockplace.getItemStack() != null && packetplayinblockplace.getItemStack().getItem() == lastMaterial && lastPacket != null && packetplayinblockplace.timestamp - lastPacket < 100) {
				lastPacket = null;
				return;
			}
		} else {
			lastMaterial = packetplayinblockplace.getItemStack() == null ? null : packetplayinblockplace.getItemStack().getItem();
			lastPacket = packetplayinblockplace.timestamp;
		}
		...
		if (packetplayinblockplace.getFace() == 255) {
			...
			if (!throttled) {
				org.bukkit.event.player.PlayerInteractEvent event = CraftEventFactory.callPlayerInteractEvent(player, Action.RIGHT_CLICK_AIR, itemstack);
			...
			// CraftBukkit end
		} else if (packetplayinblockplace.d() >= minecraftServer.getMaxBuildHeight() - 1 && (packetplayinblockplace.getFace() == 1 || packetplayinblockplace.d() >= minecraftServer.getMaxBuildHeight())) {
			ChatMessage chatmessage = new ChatMessage("build.tooHigh", new Object[] { Integer.valueOf(minecraftServer.getMaxBuildHeight()) });
			...
			if (reachDistance > (getPlayer().getGameMode() == org.bukkit.GameMode.CREATIVE ? CREATIVE_PLACE_DISTANCE_SQUARED : SURVIVAL_PLACE_DISTANCE_SQUARED))
				return;

			if (throttled || !player.playerInteractManager.interact(player, worldserver, itemstack, i, j, k, l, packetplayinblockplace.h(), packetplayinblockplace.i(), packetplayinblockplace.j())) { // Spigot - skip the event if throttled
		...
		if (itemstack == null || itemstack.n() == 0) {
			...
			Slot slot = player.activeContainer.getSlot(player.inventory, player.inventory.itemInHandIndex);

			player.activeContainer.b();
			player.g = false;
			// CraftBukkit - TODO CHECK IF NEEDED -- new if structure might not need 'always'. Kept it in for now, but may be able to remove in future
			if (!ItemStack.matches(player.inventory.getItemInHand(), packetplayinblockplace.getItemStack()) || always) {
				sendPacket(new PacketPlayOutSetSlot(player.activeContainer.windowId, slot.rawSlotIndex, player.inventory.getItemInHand()));
			}
		}
	}
	...
	public void a(IChatBaseComponent ichatbasecomponent) {
		...
		// CraftBukkit end
		c.info(player.getName() + " lost connection: " + ichatbasecomponent.c()); // CraftBukkit - Don't toString the component
		minecraftServer.az();
		...
		*/
		...
		player.n();
		String quitMessage = minecraftServer.getPlayerList().disconnect(player);
		if (quitMessage != null && quitMessage.length() > 0) {
			minecraftServer.getPlayerList().sendMessage(CraftChatMessage.fromString(quitMessage));
		}
		// CraftBukkit end
		if (minecraftServer.N() && player.getName().equals(minecraftServer.M())) {
	...
	public void sendPacket(Packet packet) {
		...
		if (true) { //NetworkManager.a(networkManager).attr(NetworkManager.protocolVersion).get() >= 17) {
		...
		if (packet instanceof PacketPlayOutChat) {
			...
			EnumChatVisibility enumchatvisibility = player.getChatFlags();
	...
	public void a(PacketPlayInHeldItemSlot packetplayinhelditemslot) {
		...
		if (packetplayinhelditemslot.c() >= 0 && packetplayinhelditemslot.c() < PlayerInventory.getHotbarSize()) {
			PlayerItemHeldEvent event = new PlayerItemHeldEvent(getPlayer(), player.inventory.itemInHandIndex, packetplayinhelditemslot.c());
			server.getPluginManager().callEvent(event);
			if (event.isCancelled()) {
				sendPacket(new PacketPlayOutHeldItemSlot(player.inventory.itemInHandIndex));
				player.v();
				return;
			}
			// CraftBukkit end

			player.inventory.itemInHandIndex = packetplayinhelditemslot.c();
			player.v();
		} else {
			c.warn(player.getName() + " tried to set an invalid carried item");
			disconnect("Invalid hotbar selection (Hacking?)"); // CraftBukkit //Spigot "Nope" -> Descriptive reason
		}
	}

	@Override
	public void a(PacketPlayInChat packetplayinchat) {
		if (player.dead || player.getChatFlags() == EnumChatVisibility.HIDDEN) { // CraftBukkit - dead men tell no tales
			...
			if (!packetplayinchat.a()) {
				try {
					minecraftServer.server.playerCommandState = true;
					handleCommand(s);
				} finally {
					minecraftServer.server.playerCommandState = false;
				}
			} else if (s.isEmpty()) {
				c.warn(player.getName() + " tried to send an empty message");
			} else if (getPlayer().isConversing()) {
				// Spigot start
				final String message = s;
				minecraftServer.processQueue.add(new Waitable() {
					@Override
					protected Object evaluate() {
				...
				// Spigot end
			} else if (player.getChatFlags() == EnumChatVisibility.SYSTEM) { // Re-add "Command Only" flag check
				ChatMessage chatmessage = new ChatMessage("chat.cannotSend", new Object[0]);
				...
				ChatMessage chatmessage1 = new ChatMessage("chat.type.text", new Object[] { player.getScoreboardDisplayName(), s });

				minecraftServer.getPlayerList().sendMessage(chatmessage1, false);
			}
			...
			if (counted && chatSpamField.addAndGet(this, 20) > 200 && !minecraftServer.getPlayerList().isOp(player.getProfile())) {
				if (packetplayinchat.a()) {
					Waitable waitable = new Waitable() {
						@Override
						protected Object evaluate() {
	...
	public void chat(String s, boolean async) {
		if (s.isEmpty() || player.getChatFlags() == EnumChatVisibility.HIDDEN)
			return;

		if (!async && s.startsWith("/")) {
			handleCommand(s);
		} else if (player.getChatFlags() == EnumChatVisibility.SYSTEM) {
	...
	private void handleCommand(String s) {
		...
		if (org.spigotmc.SpigotConfig.logCommands) {
			c.info(player.getName() + " issued server command: " + s);
		}
	...
	public void a(PacketPlayInArmAnimation packetplayinarmanimation) {
		...
		if (packetplayinarmanimation.d() == 1) {
			...
			double d1 = player.lastY + (player.locY - player.lastY) * f + 1.62D - player.height;
	...
	public void a(PacketPlayInEntityAction packetplayinentityaction) {
		...
		if (packetplayinentityaction.d() == 1) {
			...
			if (player.vehicle != null && player.vehicle instanceof EntityHorse) {
				((EntityHorse) player.vehicle).w(packetplayinentityaction.e());
			}
		} else if (packetplayinentityaction.d() == 7 && player.vehicle != null && player.vehicle instanceof EntityHorse) {
			((EntityHorse) player.vehicle).g(player);
		}
	}
	...
	public void a(PacketPlayInUseEntity packetplayinuseentity) {
		...
		player.v();
		if (entity != null) {
			boolean flag = player.hasLineOfSight(entity);
			double d0 = 36.0D;
			...
			if (player.f(entity) < d0) {
				ItemStack itemInHand = player.inventory.getItemInHand(); // CraftBukkit
				if (packetplayinuseentity.c() == EnumEntityUseAction.INTERACT) {
					...
					if (triggerLeashUpdate && (event.isCancelled() || player.inventory.getItemInHand() == null || player.inventory.getItemInHand().getItem() != Items.LEASH)) {
						// Refresh the current leash state
						sendPacket(new PacketPlayOutAttachEntity(1, entity, ((EntityInsentient) entity).getLeashHolder()));
					}

					if (triggerTagUpdate && (event.isCancelled() || player.inventory.getItemInHand() == null || player.inventory.getItemInHand().getItem() != Items.NAME_TAG)) {
						// Refresh the current entity metadata
						sendPacket(new PacketPlayOutEntityMetadata(entity.getId(), entity.datawatcher, true));
					}
					if (triggerChestUpdate && (event.isCancelled() || player.inventory.getItemInHand() == null || player.inventory.getItemInHand().getItem() != Item.getItemOf(Blocks.CHEST))) {
						sendPacket(new PacketPlayOutEntityMetadata(entity.getId(), entity.datawatcher, true));
					}
					...
					if (entity instanceof EntityItem || entity instanceof EntityExperienceOrb || entity instanceof EntityArrow || entity == player) {
	...
	public void a(PacketPlayInClientCommand packetplayinclientcommand) {
		...
		case 1:
			if (player.viewingCredits) {
				minecraftServer.getPlayerList().changeDimension(player, 0, PlayerTeleportEvent.TeleportCause.END_PORTAL); // CraftBukkit - reroute logic through custom portal management
			} else if (player.r().getWorldData().isHardcore()) {
				if (minecraftServer.N() && player.getName().equals(minecraftServer.M())) {
					player.playerConnection.disconnect("You have died. Game over, man, it\'s game over!");
					minecraftServer.U();
				} else {
					GameProfileBanEntry gameprofilebanentry = new GameProfileBanEntry(player.getProfile(), (Date) null, "(You just lost the game)", (Date) null, "Death in Hardcore");
				...
				player = minecraftServer.getPlayerList().moveToWorld(player, 0, false);
			}
			break;

		case 2:
			player.getStatisticManager().a(player);
			break;

		case 3:
			player.a(AchievementList.f);
		}
	}
	...
	public void a(PacketPlayInCloseWindow packetplayinclosewindow) {
		...
		CraftEventFactory.handleInventoryCloseEvent(player); // CraftBukkit
	...
	public void a(PacketPlayInWindowClick packetplayinwindowclick) {
		...
		player.v();
		if (player.activeContainer.windowId == packetplayinwindowclick.c() && player.activeContainer.c(player)) {
			...
			if (packetplayinwindowclick.d() == -1) {
				...
				if (packetplayinwindowclick.e() == 0 || packetplayinwindowclick.e() == 1) {
					...
					if (packetplayinwindowclick.d() == -999) {
						...
						Slot slot = player.activeContainer.getSlot(packetplayinwindowclick.d());
						if (slot != null) {
				...
				if (packetplayinwindowclick.e() >= 0 && packetplayinwindowclick.e() < 9) {
					...
					Slot clickedSlot = player.activeContainer.getSlot(packetplayinwindowclick.d());
					if (clickedSlot.isAllowed(player)) {
						ItemStack hotbar = player.inventory.getItem(packetplayinwindowclick.e());
				...
				itemstack = player.activeContainer.clickItem(packetplayinwindowclick.d(), packetplayinwindowclick.e(), 5, player);
			} else if (packetplayinwindowclick.h() == 6) {
				click = ClickType.DOUBLE_CLICK;
				action = InventoryAction.NOTHING;
				if (packetplayinwindowclick.d() >= 0 && player.inventory.getCarried() != null) {
					ItemStack cursor = player.inventory.getCarried();
					action = InventoryAction.NOTHING;
			...
			if (packetplayinwindowclick.h() != 5) {
				...
				case ALLOW:
				case DEFAULT:
					itemstack = player.activeContainer.clickItem(packetplayinwindowclick.d(), packetplayinwindowclick.e(), packetplayinwindowclick.h(), player);
					break;
				case DENY:
					...
					case SWAP_WITH_CURSOR:
						...
						player.playerConnection.sendPacket(new PacketPlayOutSetSlot(player.activeContainer.windowId, packetplayinwindowclick.d(), player.activeContainer.getSlot(packetplayinwindowclick.d()).getItem()));
						break;
			...
			if (ItemStack.matches(packetplayinwindowclick.g(), itemstack)) {
				player.playerConnection.sendPacket(new PacketPlayOutTransaction(packetplayinwindowclick.c(), packetplayinwindowclick.f(), true));
				player.g = true;
				player.activeContainer.b();
				player.broadcastCarriedItem();
				player.g = false;
			} else {
				n.a(player.activeContainer.windowId, Short.valueOf(packetplayinwindowclick.f()));
				player.playerConnection.sendPacket(new PacketPlayOutTransaction(packetplayinwindowclick.c(), packetplayinwindowclick.f(), false));
				player.activeContainer.a(player, false);
				ArrayList arraylist = new ArrayList();

				for (int i = 0; i < player.activeContainer.c.size(); ++i) {
					arraylist.add(((Slot) player.activeContainer.c.get(i)).getItem());
				}
	...
	public void a(PacketPlayInEnchantItem packetplayinenchantitem) {
		player.v();
		if (player.activeContainer.windowId == packetplayinenchantitem.c() && player.activeContainer.c(player)) {
			player.activeContainer.a(player, packetplayinenchantitem.d());
			player.activeContainer.b();
		}
	}
	...
	public void a(PacketPlayInSetCreativeSlot packetplayinsetcreativeslot) {
		if (player.playerInteractManager.isCreative()) {
			...
			if (flag || flag1 && !ItemStack.matches(player.defaultContainer.getSlot(packetplayinsetcreativeslot.c()).getItem(), packetplayinsetcreativeslot.getItemStack())) { // Insist on valid slot
			...
			if (flag1 && flag2 && flag3) {
				if (itemstack == null) {
					player.defaultContainer.setItem(packetplayinsetcreativeslot.c(), (ItemStack) null);
				} else {
					player.defaultContainer.setItem(packetplayinsetcreativeslot.c(), itemstack);
				}

				player.defaultContainer.a(player, true);
			} else if (flag && flag2 && flag3 && x < 200) {
				x += 20;
				EntityItem entityitem = player.drop(itemstack, true);
	...
	public void a(PacketPlayInTransaction packetplayintransaction) {
		if (player.dead)
			return; // CraftBukkit
		Short oshort = (Short) n.get(player.activeContainer.windowId);

		if (oshort != null && packetplayintransaction.d() == oshort.shortValue() && player.activeContainer.windowId == packetplayintransaction.c() && !player.activeContainer.c(player)) {
	...
	public void a(PacketPlayInUpdateSign packetplayinupdatesign) {
		...
		if (worldserver.isLoaded(packetplayinupdatesign.c(), packetplayinupdatesign.d(), packetplayinupdatesign.e())) {
			...
			if (tileentity instanceof TileEntitySign) {
				...
				if (!tileentitysign.a() || tileentitysign.b() != player) {
					minecraftServer.warning("Player " + player.getName() + " just tried to change non-editable sign");
					sendPacket(new PacketPlayOutUpdateSign(packetplayinupdatesign.c(), packetplayinupdatesign.d(), packetplayinupdatesign.e(), tileentitysign.lines)); // CraftBukkit
					return;
				}
			}
			...
			if (tileentity instanceof TileEntitySign) {
				...
				SignChangeEvent event = new SignChangeEvent(player.getWorld().getBlockAt(j, k, i), server.getPlayer(this.player), packetplayinupdatesign.f());
				server.getPluginManager().callEvent(event);
	...
	public void a(PacketPlayInAbilities packetplayinabilities) {
		...
		if (player.abilities.canFly && player.abilities.isFlying != packetplayinabilities.isFlying()) {
			PlayerToggleFlightEvent event = new PlayerToggleFlightEvent(server.getPlayer(player), packetplayinabilities.isFlying());
			server.getPluginManager().callEvent(event);
			if (!event.isCancelled()) {
	...
	public void a(PacketPlayInTabComplete packetplayintabcomplete) {
		...
		Iterator iterator = minecraftServer.a(player, packetplayintabcomplete.c()).iterator();
		...
		player.playerConnection.sendPacket(new PacketPlayOutTabComplete((String[]) arraylist.toArray(new String[arraylist.size()])));
	}

	@Override
	public void a(PacketPlayInSettings packetplayinsettings) {
		player.a(packetplayinsettings);
	}
		...
		if ("MC|BEdit".equals(packetplayincustompayload.c())) {
			...
			try {
				...
				if (itemstack != null) {
					...
					itemstack1 = player.inventory.getItemInHand();
					if (itemstack1 == null)
						return;
			...
			if ("MC|TrSel".equals(packetplayincustompayload.c())) {
				...
				if (!minecraftServer.getEnableCommandBlock()) {
					player.sendMessage(new ChatMessage("advMode.notEnabled", new Object[0]));
				} else if (player.a(2, "") && player.abilities.canInstantlyBuild) {
					...
					try {
						...
						if (b0 == 0) {
							TileEntity tileentity = player.world.getTileEntity(packetdataserializer.readInt(), packetdataserializer.readInt(), packetdataserializer.readInt());

							if (tileentity instanceof TileEntityCommand) {
								commandblocklistenerabstract = ((TileEntityCommand) tileentity).getCommandBlock();
							}
						} else if (b0 == 1) {
							Entity entity = player.world.getEntity(packetdataserializer.readInt());
						...
						if (commandblocklistenerabstract != null) {
							...
							player.sendMessage(new ChatMessage("advMode.setCommand.success", new Object[] { s }));
						}
						// CraftBukkit start
				...
				if (player.activeContainer instanceof ContainerBeacon) {
					try {
						datainputstream = new DataInputStream(new ByteArrayInputStream(packetplayincustompayload.e()));
						i = datainputstream.readInt();
						int j = datainputstream.readInt();
						ContainerBeacon containerbeacon = (ContainerBeacon) player.activeContainer;
			...
			} else if ("MC|ItemName".equals(packetplayincustompayload.c()) && player.activeContainer instanceof ContainerAnvil) {
				ContainerAnvil containeranvil = (ContainerAnvil) player.activeContainer;
	...
	public boolean isDisconnected() {
		return !player.joining && !NetworkManager.a(networkManager).config().isAutoRead();
	}
	// CraftBukkit end
}
