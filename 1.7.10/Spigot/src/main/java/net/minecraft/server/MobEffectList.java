...
public class MobEffectList {
    ...
    // Alkazia - new potion effect, yes falling :)
    public static final MobEffectList fall = (new MobEffectList(24, false, 3426654)).b("potion.fall").b(3, 2);
	...
	public void tick(EntityLiving entityliving, int i) {
		...
		} else if (id == HUNGER.id && entityliving instanceof EntityHuman) {
			((EntityHuman) entityliving).applyExhaustion(0.025F * (i + 1));
		} else if (id == SATURATION.id && entityliving instanceof EntityHuman) {
		...
		} else if ((id != HEAL.id || entityliving.aR()) && (id != HARM.id || !entityliving.aR())) {
			if (id == HARM.id && !entityliving.aR() || id == HEAL.id && entityliving.aR()) {
				entityliving.damageEntity(DamageSource.MAGIC, 3 << i);
			}
		} else {
	...
	public void applyInstantEffect(EntityLiving entityliving, EntityLiving entityliving1, int i, double d0, EntityPotion potion) {
		...
		if ((id != HEAL.id || entityliving1.aR()) && (id != HARM.id || !entityliving1.aR())) {
			if (id == HARM.id && !entityliving1.aR() || id == HEAL.id && entityliving1.aR()) {
				j = (int) (d0 * (3 << i) + 0.5D);
	...
	public MobEffectList a(IAttribute iattribute, String s, double d0, int i) {
		...
		I.put(iattribute, attributemodifier);
		return this;
	}
