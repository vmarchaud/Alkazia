...
public class CrashReport {
	...
	private void h() {
		d.a("Minecraft Version", new CrashReportVersion(this));
		d.a("Operating System", new CrashReportOperatingSystem(this));
		d.a("Java Version", new CrashReportJavaVersion(this));
		d.a("Java VM Version", new CrashReportJavaVMVersion(this));
		d.a("Memory", new CrashReportMemory(this));
		d.a("JVM Flags", new CrashReportJVMFlags(this));
		d.a("AABB Pool Size", new CrashReportAABBPoolSize(this));
		d.a("IntCache", new CrashReportIntCacheSize(this));
		d.a("CraftBukkit Information", new org.bukkit.craftbukkit.CraftCrashReport()); // CraftBukkit
	}
	...
	public void a(StringBuilder stringbuilder) {
		if ((h == null || h.length <= 0) && e.size() > 0) {
			h = ArrayUtils.subarray(((CrashReportSystemDetails) e.get(0)).a(), 0, 1);
		}

		if (h != null && h.length > 0) {
			stringbuilder.append("-- Head --\n");
			stringbuilder.append("Stacktrace:\n");
			StackTraceElement[] astacktraceelement = h;
	...
	public String d() {
		...
		if (((Throwable) object).getMessage() == null) {
			if (object instanceof NullPointerException) {
				object = new NullPointerException(b);
			} else if (object instanceof StackOverflowError) {
				object = new StackOverflowError(b);
			} else if (object instanceof OutOfMemoryError) {
				object = new OutOfMemoryError(b);
			}

			((Throwable) object).setStackTrace(c.getStackTrace());
		}
	...
	public CrashReportSystemDetails a(String s, int i) {
		...
		if (g) {
			...
			StackTraceElement[] astacktraceelement = c.getStackTrace();
			...
			g = crashreportsystemdetails.a(stacktraceelement, stacktraceelement1);
			if (j > 0 && !e.isEmpty()) {
				CrashReportSystemDetails crashreportsystemdetails1 = (CrashReportSystemDetails) e.get(e.size() - 1);

				crashreportsystemdetails1.b(j);
			} else if (astacktraceelement != null && astacktraceelement.length >= j && 0 <= k && k < astacktraceelement.length) {
				h = new StackTraceElement[astacktraceelement.length - j];
				System.arraycopy(astacktraceelement, 0, h, 0, h.length);
			} else {
				g = false;
			}
		}

		e.add(crashreportsystemdetails);
		return crashreportsystemdetails;
	}

	private static String i() {
		String[] astring = new String[] { "Who set us up the TNT?", "Everything\'s going to plan. No, really, that was supposed to happen.", "Uh... Did I do that?", "Oops.", "Why did you do that?", "I feel sad now :(", "My bad.", "I\'m sorry, Dave.", "I let you down. Sorry :(", "On the bright side, I bought you a teddy bear!", "Daisy, daisy...", "Oh - I know what I did wrong!",
