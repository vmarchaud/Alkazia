...
class CraftMetaFirework extends CraftMetaItem implements FireworkMeta {
	...
	CraftMetaFirework(CraftMetaItem meta) {
		...
		if (that.hasEffects()) {
			effects = new ArrayList<FireworkEffect>(that.effects);
		}
	}
	...
	boolean equalsCommon(CraftMetaItem meta) {
		...
		if (meta instanceof CraftMetaFirework) {
			...
			return (hasPower() ? that.hasPower() && power == that.power : !that.hasPower()) && (hasEffects() ? that.hasEffects() && effects.equals(that.effects) : !that.hasEffects());
		}
	...
	public CraftMetaFirework clone() {
		...
		if (effects != null) {
			meta.effects = new ArrayList<FireworkEffect>(effects);
		}
