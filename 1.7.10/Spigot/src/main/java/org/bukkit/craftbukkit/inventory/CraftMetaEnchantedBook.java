...
class CraftMetaEnchantedBook extends CraftMetaItem implements EnchantmentStorageMeta {
	...
	CraftMetaEnchantedBook(CraftMetaItem meta) {
		...
		if (that.hasEnchants()) {
			enchantments = new HashMap<Enchantment, Integer>(that.enchantments);
		}
	}
	...
	boolean equalsCommon(CraftMetaItem meta) {
		...
		if (meta instanceof CraftMetaEnchantedBook) {
			...
			return hasStoredEnchants() ? that.hasStoredEnchants() && enchantments.equals(that.enchantments) : !that.hasStoredEnchants();
		}
		return true;
	}
