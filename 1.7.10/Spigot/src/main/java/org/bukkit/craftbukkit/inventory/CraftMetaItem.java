...
class CraftMetaItem implements ItemMeta, Repairable {
	...
	public static class SerializableMeta implements ConfigurationSerializable {
		...
		static {
			classMap = ImmutableMap.<Class<? extends CraftMetaItem>, String> builder().put(CraftMetaBook.class, "BOOK").put(CraftMetaSkull.class, "SKULL").put(CraftMetaLeatherArmor.class, "LEATHER_ARMOR").put(CraftMetaMap.class, "MAP").put(CraftMetaPotion.class, "POTION").put(CraftMetaEnchantedBook.class, "ENCHANTED").put(CraftMetaFirework.class, "FIREWORK")
	...
	CraftMetaItem(CraftMetaItem meta) {
		...
		if (meta.hasLore()) {
			lore = new ArrayList<String>(meta.lore);
		}

		if (meta.hasEnchants()) {
			enchantments = new HashMap<Enchantment, Integer>(meta.enchantments);
		}
	...
	boolean equalsCommon(CraftMetaItem that) {
		return (hasDisplayName() ? that.hasDisplayName() && displayName.equals(that.displayName) : !that.hasDisplayName()) && (hasEnchants() ? that.hasEnchants() && enchantments.equals(that.enchantments) : !that.hasEnchants()) && (hasLore() ? that.hasLore() && lore.equals(that.lore) : !that.hasLore())
				&& (hasAttributes() ? that.hasAttributes() && attributes.equals(that.attributes) : !that.hasAttributes()) && (hasRepairCost() ? that.hasRepairCost() && repairCost == that.repairCost : !that.hasRepairCost()) && spigot.isUnbreakable() == that.spigot.isUnbreakable(); // Spigot
	}
	...
	public CraftMetaItem clone() {
		try {
			...
			if (lore != null) {
				clone.lore = new ArrayList<String>(lore);
			}
			if (enchantments != null) {
				clone.enchantments = new HashMap<Enchantment, Integer>(enchantments);
			}
			return clone;
