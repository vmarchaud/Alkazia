...
public enum Rel {
    LEADER(70, "your faction leader", "your faction leader", "", ""), OFFICER(60, "an officer in your faction", "officers in your faction", "", ""), MEMBER(50, "a member in your faction", "members in your faction", "your faction", "your factions"), RECRUIT(45, "a recruit in your faction", "recruits in your faction", "", ""), ALLY(40, "an ally", "allies", "an allied faction", "allied factions"), TRUCE(30, "someone in truce with you", "those in truce with you", "a faction in truce", "factions in truce"), NEUTRAL(20, "someone neutral to you", "those neutral to you", "a neutral faction", "neutral factions"), ENEMY(10, "an enemy", "enemies", "an enemy faction", "enemy factions"), ;
    ...
    public static Rel parse(String str) {
        ...
        if (str.equals("admin")) return LEADER;

        if (str.equals("moderator")) return OFFICER;

        if (str.equals("normal")) return MEMBER;
    ...
    public boolean isAtLeast(final Rel rel) {
        return this.value >= rel.value;
    }

    public boolean isAtMost(final Rel rel) {
        return this.value <= rel.value;
    }

    public boolean isLessThan(final Rel rel) {
        return this.value < rel.value;
    }

    public boolean isMoreThan(final Rel rel) {
        return this.value > rel.value;
    }

    public ChatColor getColor() {
        if (this.isAtLeast(RECRUIT)) return Conf.colorMember;
        else if (this == ALLY) return Conf.colorAlly;
        else if (this == NEUTRAL) return Conf.colorNeutral;
        else if (this == TRUCE) return Conf.colorTruce;
        else return Conf.colorEnemy;
    }

    public String getPrefix() {
        if (this == LEADER) return Conf.prefixLeader;

        if (this == OFFICER) return Conf.prefixOfficer;

        if (this == MEMBER) return Conf.prefixMember;

        if (this == RECRUIT) return Conf.prefixRecruit;
    ...
    public double getRelationCost() {
        if (this == ENEMY) return Conf.econCostEnemy;
        else if (this == ALLY) return Conf.econCostAlly;
        else if (this == TRUCE) return Conf.econCostTruce;
